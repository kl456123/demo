...
router
  ...
  .post(pChecker.passIf('member','login'),checkReset,function(req,res){
    ...
    var obj = new Model();
    if(pwd===newPwd) {
      return res.rd.errorBack('新密码与原密码一致',req.xhr);
    }
    obj.get({cellphone:req.body.cellphone},function(err,obj){
      if(err) {
        return res.rd.errorBack('用户不存在',req.xhr);
      }
      var pwd = tool.hashMD5(pwd);
      if(pwd!==obj.password) {
        return res.rd.errorBack('密码错误，修改失败',req.xhr);
      }
      obj.password = tool.hashMD5(newPwd);
      logger.trace('new password:',newPwd);
      obj.save(function(err,obj){
        if(err) {
          return res.rd.errorBack('数据库错误',req.xhr);
        }
