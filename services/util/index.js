...
var handleUpImgs = exports.handleUpImgs = function(html, refer){
  ...
  if(showTempPaths.length === 0) return html;
  tempToReferUpDir(showTempPaths, refer);
  var html = replaceImgSrc(html, refer);
...
var matchImgSrc = function(html){
  ...
  if(matched === null) return showPaths;
...
var tempToReferUpDir = function(tempPaths, refer){
  ...
  tempPaths.forEach(function(o, i){
    ...
    var oldPath = tempUpDir+'/'+filename; //'./public/images/upload/richText/temp/filename'
    var newPath = realUpDir+'/'+filename; //'./public/images/upload/richText/uuid/filename'
    if(fs.existsSync(oldPath)) fs.renameSync(oldPath, newPath);  //rename
  });
}
...
var removeNoReferImgs = function(html, refer){
  ...
  files.forEach(function(o, i){
    ...
    if(html.indexOf(o) === -1) fs.unlinkSync(filePath);
  });
};
